...
- **Review round-3 tests (23 new, all TDD):** `StaxCsdlParserEnumMemberValueTest` 2 (CSDL previous+1 enum values), `GeneratorSchemaInfoAggregationTest` 2 (aggregate `ServiceSchemaInfo` per output package), `AbstractHierarchyDeserializationTest` 2 (compile **and deserialize** abstract hierarchies — base props survive), `EntityOperationsEmptyBodyTest` 3 (PATCH/GET on 204 → null), `ContextPathTest` +2 (literal `+` in nextLink values), `GuidPropertyTest` 7 + `GeneratorGuidPropertyTest` 3 (unquoted GUID filter literals), `RequestGeneratorCollectionComplexNavTest` 2 (collection-of-complex navs skipped)
- **Total: 562 tests passing** (165 core + 289 runtime + 16 maven + 92 test module; 440 offline by default, live via `-Plive-tests`)
- **Future:** Cancellable streaming, Content-ID resolution in changesets, remove duplicate `JavaNetHttpTransport`, wire `changedFields` into partial PATCH, constant-name dedupe for case-colliding properties (`budget` vs `Budget`), cache the interceptor chain per `Context`
