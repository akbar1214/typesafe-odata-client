...
### 97. Lambda Query API (Plan D): Selector Lambdas on a Sealed `Expandable`, `NavProperty` Deleted

**Decision:** Every read-shaping option on generated requests — `select`, `orderBy`, `expand`, `filter` (`select`/`expand` only on entity requests) — gains a **selector-lambda overload** alongside its constant form: `filter(p -> p.FIRST_NAME.equalTo("Scott"))`, `expand(p -> p.TRIPS.select(t -> t.NAME).top(2))`. The lambdas apply to a generated per-entity `Selector` inner class whose fields SHARE the entity's constant instances (`public final StringProperty<Person> FIRST_NAME = Person.FIRST_NAME;`; inherited properties are constructed inline — their constants live on the declaring base class). The runtime deletes `NavProperty` entirely: its builder methods move onto (a) the record `NavQuery<S, T, Sel>` — 10th component `Supplier<Sel> selectorFactory`, static factories `of(name[, factory])`/`raw(...)`, `as()` 2-arg (nulls the factory) / 3-arg (swaps it) — and (b) `CollectionProperty<E, T, F, Sel>` (4th type param appended; ≤4-arg constructors delegate with a null factory). Both implement the sealed `Expandable<E>` (`permits NavQuery, CollectionProperty`), so the generated requests' two legacy `expand` overloads collapse into ONE `expand(Expandable<? super E>...)` plus the lambda form — the twin lambda expands of the parent proposal were a HARD name clash (same erasure, JLS 4.6/8.4.2: `Function<S, NavProperty<...>>` vs `Function<S, NavQuery<...>>`), and removing `NavProperty` from the world is what makes a single lambda `expand` legal.

**Why class-level `Sel`:** a method-level generic cannot type a selector lambda — inference flows from the target type into the lambda, never the reverse, so `<Sel> select(Function<Sel, ...>)` collapses `Sel` to `Object`. With `Sel` nailed down in the receiver's type, full-depth nesting composes because each hop's factory arrives WITH the value: the request builds `new Person.Selector()` concretely → `p.TRIPS` carries `Trip.Selector::new` → `.select(t -> ...)` propagates it → `.expand(u -> ...)` applies the carried factory → `u.PLAN_ITEMS` carries hop 3's factory.

**Details:**
- Factory rules (uniform with the `filterableFactory` precedent): generated nav constants wire the factory (`NavQuery.of("Photo", Photo.Selector::new)`; `new CollectionProperty<>(..., Trip.Filterable::new, Trip.Selector::new)`; cast constants use the 3-arg `as(..., Sub.Selector::new)`); factory-less construction (`NavQuery.of(name)`, `raw(...)`, 2-arg `as()`, hand-built ≤4-arg `CollectionProperty`) chains CONSTANTS fine but fails fast on lambda overloads with `IllegalStateException` (same shape as `any()`'s).
- Zero-arg varargs calls became AMBIGUOUS once both a constant and a lambda varargs overload exist (`select()` matches both in the varargs phase) — explicit no-arg bridge methods restore the old surface, and a bridge is needed for EVERY varargs pair whose zero-arg form was legal: five in total (`select`/`orderBy` on both runtime classes, `select`/`orderBy` on collection requests, `select` on entity requests; `expand` needs none — its zero-arg form was already ambiguous under the twin `NavProperty`/`NavQuery` overloads, so nothing regressed there). The first pass bridged only `select()`/`expand()` at runtime and overclaimed restoration; review caught `orderBy()` and the entire request level. Overload pairs with ≥1 argument are unambiguous (different erasures).
- `Filterable`-inner fields declare `CollectionProperty<..., ?>` (wildcard `Sel` — they serve `any`/`all`, never selector lambdas; the wildcard is honest about it). Complex-element collections are wildcarded too: complex types get NO `Selector` (no request class targets them), so complex-target navigations (possible through typedef chains) are now skipped from nav-constant emission entirely.
- `Names` shadow list: `NavProperty` removed (the class is gone — an entity named `NavProperty` is legal again); `NavQuery`/`Expandable` added.
- `NavQuery` record equality is meaningless with a `Supplier` component (method-ref identity is unspecified and may be cached) — the convention test pins equal RENDERING and never asserts instance equality in either direction.
- Breaking (pre-1.0, no release cut): `NavProperty` is deleted and `NavQuery`/`CollectionProperty` gained type parameters — code NAMING those types must be updated. Call sites that only USE the constants recompile unchanged (all 27 generated-client `.expand(...)` call sites did; constants kept their names). Design doc: `LAMBDA_QUERY_API_PLAN_D.md` (§1 erasure probe; §3 D1 rule).

**Reason:** Constants already are the metamodel; selector lambdas give one consistent mental model across read shaping and make full-depth polymorphic chains readable (`expand(p -> p.TRIPS.select(t -> t.NAME).expand(u -> u.PLAN_ITEMS))`), with every cross-entity mistake a compile error in both spellings (`p.FLIGHTS` is not a member of `Person.Selector`). Verified by `NavQueryExpandTest` 31 (rewritten from `NavPropertyExpandTest`), `ExpandableTest` 3, `CollectionPropertyTypedLambdaTest` 19 (render-equivalence lambda vs constant, full-depth composition, fail-fast without factory, `as()` factory swap/null, convention pin), `NavQueryValidationTest` 4 (fixture migrated), `EntityGeneratorSelectorTest` 7, `RequestGeneratorLambdaTest` 6 (incl. the zero-arg bridge pin), `LambdaQueryCompilationTest` 1 (full-depth chains + 3-arg `as()` + cast constants compile against the regenerated TripPin client), `QueryTypeSafetyCompilationTest.crossEntityLambdaFailsToCompile` (negative compile), `LambdaConstantUrlEquivalenceTest` 5 (lambda-built chains render byte-identical URLs to their constant twins, offline via `buildContext().toRelativeUrl()`); `NavQueryExpandTest` also pins the runtime `orderBy()` bridges. Reactor 888 offline (470 runtime + 380 core + 25 maven + 13 test, clean-surefire measured) / 1012 live.
...
- **Base-chain robustness tests (Batch A):** `BaseChainCycleTest` 4 + `BoundAncestorCycleTest` 1 (timeout-bounded) + `RedeclaredMemberTest` 6 (own-wins merge, compile referee, prop/nav/mid-chain loud failures) + `OpenRootNamespaceTest` 1 (exact-schema open roots) + `RequestGeneratorBaseParityTest` 1 (unqualified cross-schema `findBase`)
...
- **Total: 888 tests passing offline by default** (380 core + 470 runtime + 25 maven + 13 test module; the remaining generated-client/live classes run via `-Plive-tests` — 1012 total incl. live)
...
187. **Profile the generator at scale before assuming past perf fixes covered everything — per-entity caches must be shared, not rebuilt.** Lesson 178 hoisted per-entity generator construction but missed `new OperationGenerator(...)` inside `RequestGenerator.generateEntityRequest`: every one of 15k entities rebuilt the bound index and ancestor chains from scratch, and `scanEntityType` concatenated `namespace + "." + name` per comparison for an O(allTypes) scan per distinct ancestor link. async-profiler (`run-codegen-bench.sh`, 15k entities / 48k files) put ancestor resolution at ~48% of all generation allocation and `boundOperationsFor` at ~90% of entity-request cost. Fixes: a qualified-name entity index built once per `OperationGenerator` (same order/semantics as the scan, including the ambiguous-unqualified throw), plus one shared `OperationGenerator` per `RequestGenerator` instance (only when constructed with the full schema list; single-schema test constructions keep per-call behavior). Measured: 8.3s -> 4.8s per 15k-entity generation (-42%, identical file output). Rule: any `new XGenerator(...)` inside a per-type loop is O(entities) cache rebuilds until proven shared — grep for it. Remaining on the profile (not yet taken): `Names` sanitizers are pure functions of the CSDL name (~7% — cache per unique name) and `checkChainConflicts`+merges re-walk leaf entities that cannot conflict (base==null fast path).
188. **A warning-coverage claim must be verified by enumerating every skip site, and quoted test counts must be recomputed from the branch — not carried over.** Review of the hygiene batch found three member-level loops (`parseKey`, `parseNavigationProperty`, `parseEnumType`) with no `default` branch at all — unknown children fell through silently, unwarned AND unconsumed (nested content leaked into the sibling loop), while the PR body claimed every site was covered; the fix is the same `warnIgnored` + `skipElement` shape with the `Annotation` exemption (one choke point, so legal inline annotations stay silent everywhere). Same review caught the totals line and the PR-body decomposition pasted from three PRs prior (422 + 25 was the runtime count plus the plugin count, not the delta). Rules: when the fix is 'every X', grep for the complement class (here: child loops without a `default`), not the sites already fixed; and recompute every quoted number from the current tree (`mvn test` summaries + `git log` counts), including arithmetic decompositions — a correct headline with a wrong decomposition still misleads. Verified by `StaxCsdlParserWarningsTest` +3 (Key/Nav/Enum typo warnings); reactor 846 offline (366 core + 447 runtime) / 970 live.

189. **Selector-lambda overloads: the type parameter must live on the RECEIVER, zero-arg varargs calls become ambiguous, and every closed-world assumption in the renderer must be re-inventoried when a class is deleted.** Implementing Plan D (decision 97) surfaced four non-obvious mechanics beyond the headline erasure clash: (a) a method-level generic CANNOT type a selector lambda — inference flows from the target type into the lambda, never the reverse, so `<Sel> select(Function<Sel,...>)` collapses `Sel` to `Object` and `t.NAME` stops resolving; the only fix is `Sel` as a CLASS-level parameter propagated through every builder (`NavQuery<S,T,Sel>`, `CollectionProperty<E,T,F,Sel>`), which then composes at full depth because each hop's factory arrives with the value. (b) With both a constant and a lambda varargs overload, the ZERO-ARG invocation matches both (`select()`) and javac reports ambiguity — neither overload is more specific when the element types are unrelated; explicit no-arg bridges (`select()` → `select(new PropertyExpression[0])`) restore the old surface, ≥1-argument calls were never ambiguous — and the first pass proved lesson 188's complement-class rule applies to CODE fixes too: it bridged `select()`/`expand()` on the runtime classes only, while `orderBy()` (both runtime classes) and `select()`/`orderBy()` on the generated requests stayed ambiguous (request-level `expand()` needed nothing — already ambiguous under the twin `NavProperty`/`NavQuery` overloads, so no regression). When the fix is 'restore X', enumerate every X (here: every constant+lambda varargs pair on every class), don't bridge the sites you remember. (c) Wiring `Selector::new` into nav constants converted a latent junk-but-compiling emission (typedef-of-COMPLEX nav constants, M4's cousin on the entity side) into an uncompilable one — complex types have no `Selector` — so the guard moved into `generateNavPropertyConstant` too; whenever a new emission REQUIRES a generated collaborator class, grep the metadata corpus for types lacking it. (d) Deleting a runtime class ripples beyond the shadow list: the deleted name becomes un-reserved (`NavProperty` legal again — expectation flip in the pinned test), the deleted class's GETTERS die with it (`getEdmName()` on nav constants → record accessor `edmName()`), and one stale unused import in a hand-written test (`import ...query.NavProperty;`) that javac never warns about becomes a hard error. Red-first note: the compile referee earned its keep twice — it caught a REAL untested hole (cast constant used inside `select()` instead of `expand()`; my own fixture bug, not the generator's) and proved the full-depth chains compile only after the emission existed. Verified by the tests listed in decision 97 (incl. the zero-arg bridge pins); reactor 888 offline / 1012 live.
